...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _boot() async {
    if (asking || scanning) return;
    setState(() => asking = true);
    PermissionState state;
    try { state = await GalleryStore.instance.request(); } catch (_) {
      if (mounted) setState(() { asking = false; error = 'Photo access unavailable. Please retry.'; });
      return;
    }
    if (!mounted) return;
    setState(() {
    ...
    if (state.hasAccess) await _scan();
  }
      ...
      body: asking || scanning
          ...
          : ListView(
              ...
              children: [
                ...
                if (error != null && permission?.hasAccess != true) Text(error!, style: const TextStyle(color: SM.toss)),
                if (permission?.hasAccess != true) ...[
                  Text('Allow access to selected photos or your library. Only accessible items are scanned.', style: SM.body.copyWith(color: SM.cream)),
                  ...
                  if (!bill.isPlus)
                    Padding(
                      ...
                      child: OutlinedButton(
                        ...
                        style: _outlineStyle(),
                        child: const Text('Pro · remove ads'),
                      ),
                    ),
                  OutlinedButton(
                    onPressed: () async {
