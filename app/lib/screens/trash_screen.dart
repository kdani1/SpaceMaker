...
class _TrashScreenState extends State<TrashScreen> {
  ...
  Future<void> _empty() async {
    ...
    if (g.trash.isEmpty) return;
    setState(() => busy = true);
    try {
      await Api.instance.consumeEmpty();
      final count = await g.emptyTrash();
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(count == 0 ? 'Nothing was deleted. Check Photos permission.' : '$count items moved to system trash.')),
      );
      setState(() {});
    } catch (e) {
      if (!mounted) return;
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text('Waiting in trash — recoverable for 30 days after you empty', style: SM.body),
              ],
            ),
          ),
          Expanded(
